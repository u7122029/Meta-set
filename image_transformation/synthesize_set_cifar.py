...
def main():
    for q in range(1,21):
        c101_path = f"C:/Users/_Kookie/Documents/git/autoeval_baselines/code/data/val_data/cifar-10-f-32/{q}"
        teset = CIFAR10NP(f"{c101_path}/data.npy", f"{c101_path}/labels.npy")
        #torchvision.datasets.CIFAR10(root='/PROJECT_DIR/dataset/',train=False, download=True, transform=te_transforms)
        ...
        # Change to your path
        try:
            os.makedirs('data/meta-set')
        except:
            print('Already have this path')
        ...
        for num in trange(num_sets):
            ...
            if not os.path.exists(f"data/meta-set/{q}/{num}"): os.makedirs(f"data/meta-set/{q}/{num}")
            np.save(f"data/meta-set/{q}/{num}/data.npy", new_data)
            np.save(f"data/meta-set/{q}/{num}/labels.npy", teset.labels)
